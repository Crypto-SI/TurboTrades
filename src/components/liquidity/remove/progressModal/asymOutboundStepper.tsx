  ...
  const fetchProgress = async(_hash: string) => {
    try {
      ...
      if (_action.type === "refund") { 
        ...
        setMayaTitle(`${token.ticker} is on its way to your wallet`);
        setMayaMessage(`${token.ticker} is on its way to your wallet via maya block ${_action.height}, it will arrive within approximately ${_renderEstimationTime(_timeEstimation)}mins`);
    ...
    <div className='w-full text-left flex text-gray-500 dark:text-gray-400'>
      ...
      <div className={`relative w-1/2 flex flex-col items-start px-7 min-h-[100px]`}>
        <span className='dark:text-gray-200'>
          ...
          { status === STATUS.SUCCESS && `${token.ticker} Reached your wallet Successfully` }
          { status === STATUS.FAILED && `MAYA Request Failed` }
        </span>
        <span className='text-[12px]'>
          { status === STATUS.READY &&  `Ready for ${token.ticker} withdraw` }
          ...
          { status === STATUS.SUCCESS &&  `${token.ticker} reached your wallet by Maya block ${blockHeight}`}
