...
const StepperItem = ({ token, hash, stepper, setStepper }: IParamsStepItem) => {
  ...
  React.useEffect(() => {
    if (txResult?.confirmed) {
      showNotification ("Withdraw Success", STATUS.SUCCESS);
  ...
  const fetchProgress = async(_hash: string) => {
    try {
      ...
      if (_action.type === "refund") { 
        ...
        setMayaTitle(`${token.ticker}/CACAO is on its way to your wallet`);
        setMayaMessage(`${token.ticker}/CACAO is on its way to your wallet via maya block ${_action.height}, it will arrive within approximately ${_renderEstimationTime(_timeEstimation)}mins`);
        ...
        if (_action.out.length !== 2) throw "not yet outbound transaction";
        console.log("withdraw success");
        setOutboundHash(_action.out[1].txID);
        setMayaMessage(`${token.ticker}/CACAO reached by maya block ${_action.height}`);
    ...
    <div className='w-full text-left flex text-gray-500 dark:text-gray-400'>
        ...
        { status === STATUS.SUCCESS && txResult &&
          <>
            ...
            <div className='text-[12px] flex gap-1 items-center pb-1'>
              <span className='cursor-pointer hover:underline' onClick={() => window.open(CHAIN_DATA[token.chain].explorer + '/tx/' + outboundHash, "_blank")}>{reduceAddress(outboundHash, 8)}</span>
            ...
            { txResult.fee > 0 && <div className='text-[12px]'>fee: {txResult.fee}{token.ticker}</div> }
          </> 
        }
      </div>
      <div className={`relative w-1/2 flex flex-col items-start px-7 min-h-[100px]`}>
        <span className='dark:text-gray-200'>
          ...
          { status === STATUS.SUCCESS && `Payment Reached your wallet Successfully` }
          { status === STATUS.FAILED && `MAYA Request Failed` }
        </span>
        <span className='text-[12px]'>
          { status === STATUS.READY &&  `Ready for ${token.ticker}/CACAO withdraw` }
          { status === STATUS.PENDING &&  
            (!mayaMessage ? `Withdraw will be processed within approximately${timeEstimation ? ` ${_renderEstimationTime(timeEstimation)}mins` : '...'}` : mayaMessage)
          }  
