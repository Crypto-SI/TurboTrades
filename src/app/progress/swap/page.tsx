...
const Progress = () => {
  ...
  const _fetchProgress = async() => {
    try {
      ...
      const [action] = [...actions].reverse(); //get the last element
      if (action.type === "swap" && action.status === "success") {
        showNotification ("Successfully Swap", "info");
        statusRef.current = "success";
        setResult("Swap Success")
        setAction (action);
      } else if (action.type === "refund") {
        statusRef.current = "refund";
      ...
      <div className="rounded-2xl p-[1px] bg-gradient-to-tr from-[#ff6a0096] via-[#6d78b280] to-[#e02d6f86] mt-10 md:mt-0 w-full lg:w-[500px]">
        <div className="rounded-2xl p-4 bg-white dark:bg-[#0A0C0F] dark:text-white text-black">
          ...
          <div className="bg-[#F3F7FC] dark:text-[#dbdde0] dark:bg-[#030506] w-full rounded-xl mt-1 p-5">
            ...
            {/* in transaction */}
            { _renderTxInfo ("In Address", String(params.from), TOKEN_DATA[String(params.in)].explorer + '/address/' + params.from) }
            { _renderTxInfo ("In TxHash", String(params.hash), TOKEN_DATA[String(params.in)].explorer + '/tx/' + params.hash) } 
            {
              action && 
              <div className='flex justify-between items-center text-sm my-3 mb-6'>
                <div className=''>In Time</div>
                <div className='flex gap-1 items-center pr-2'>
                  { moment(Number(params.start)).format('YYYY:MM:DD HH:MM') }
                </div>
              </div>
            }
            {/* out transactions.... */}
            {
              action ?
              <>
                { _renderTxInfo ("Out Address", action.out[0].address, TOKEN_DATA[action.out[0].coins[0].asset].explorer + '/address/' + action.out[0].address) }
                { _renderTxInfo("Out TxHash", action.out[0].txID ? action.out[0].txID : params.hash, TOKEN_DATA[action.out[0].coins[0].asset].explorer + '/tx/' + (action.out[0].txID ? action.out[0].txID : params.hash)) }
                <div className='flex justify-between items-center text-sm my-3 mb-6'>
                  <div className=''>In Time</div>
                  <div className='flex gap-1 items-center pr-2'>
                    { moment(Number(action.date) / 1000000).format('YYYY:MM:DD HH:MM') }
                  </div>
                </div>
              </> :
              <div>
